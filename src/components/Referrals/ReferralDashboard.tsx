      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-3">
            ...
            {pointsToNextCourse < 10 && (
              <p className="text-sm text-gray-600">
                Jeszcze {pointsToNextCourse} {pointsToNextCourse === 1 ? 'punkt' : 'punktów'} do kolejnego darmowego kursu
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">
            Poleceni użytkownicy ({referredUsers.length})
          </h2>
          {loading ? (
            ...
            <div className="divide-y">
              {referredUsers.map(user => (
                <div key={user.uid} className="py-3">
                  ...
                  <p className="text-sm text-gray-500">
                    Dołączył: {user.createdAt?.toLocaleDateString()}
                  </p>
                </div>
              ))}
            </div>
